...
import { Upload, RefreshCw, Trash2, Edit2, Check, X } from 'lucide-react';
...
const SummaryEditRow: React.FC<{
  ...
  const key = `${row.ro_number}|||${row.location}`;
  ...
  return (
    <tr className={TABLE_CLASSES.tr}>
      <td className={TABLE_CLASSES.td}><span className='text-xs font-mono text-zinc-600 dark:text-zinc-400'>{row.ro_number}</span></td>
      <td className={TABLE_CLASSES.td}><span className={TABLE_CLASSES.tdText}>{formatDate(row.po_date)}</span></td>
      ...
      <td className={TABLE_CLASSES.td}><span className={TABLE_CLASSES.tdText}>{row.accepted_qty.toLocaleString()}</span></td>
...
export default function BlinkitShipmentProcessing() {
  ...
  const loadProcessing = useCallback(async () => {
    ...
    setErrorProc(null);
    try {
      const res = await axios.get(`${API_URL}/blinkit_shipment/processing`);
  ...
  const loadSummary = useCallback(async () => {
    ...
    setErrorSum(null);
    try {
      const res = await axios.get(`${API_URL}/blinkit_shipment/summary`);
  ...
  const handleUploadProc = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await axios.post(`${API_URL}/blinkit_shipment/processing/upload`, fd);
  ...
  const handleClearProc = useCallback(async () => {
    ...
    try {
      const res = await axios.delete(`${API_URL}/blinkit_shipment/processing`);
  ...
  const saveSummaryEdit = useCallback(async (key: string, updates: Partial<SummaryRow>) => {
    const [roNumber, location] = key.split('|||');
    try {
      await axios.put(
        `${API_URL}/blinkit_shipment/summary/${encodeURIComponent(roNumber)}`,
        updates,
        { params: { location } },
      );
      setSummary(prev => prev.map(r =>
        `${r.ro_number}|||${r.location}` === key ? { ...r, ...updates } : r
      ));
      toast.success('Saved');
    } catch {
    ...
    <div className='space-y-4'>
      ...
      <div className={TABLE_CLASSES.container}>
        <div className={TABLE_CLASSES.headerSection}>
          <h2 className='text-xl font-semibold text-zinc-900 dark:text-zinc-100'>Blinkit Shipment Processing</h2>
        ...
        <div className='space-y-4'>
          <div className={TABLE_CLASSES.container}>
            <div className={TABLE_CLASSES.headerSection}>
              <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
                <div>
                  <p className='text-sm text-zinc-600 dark:text-zinc-400'>
                    Upload your Blinkit shipment plan. Status field controls how qty is counted in the Planning page
                    (<strong>processing → requested qty</strong>, <strong>packed/intransit → packed qty</strong>).
                  </p>
                  <p className='text-xs text-zinc-400 mt-1'>
                    Expected columns: RO Number, Date, Location, Item ID, UPC/EAN, SKU Code, Item Name, MRP, SP, Requested Qty, Packed Qty, Status
                  </p>
                </div>
              ...
              <div className='mt-4'>
                <SearchBar value={searchProc} onChange={setSearchProc} placeholder='Search by RO Number, SKU, item or location…' />
              </div>
              {processing.length > 0 && (
          ...
          {!loadingProc && !errorProc && processing.length === 0 && (
            <div className='bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-lg p-8 text-center text-sm text-zinc-500 dark:text-zinc-400'>
              No processing data yet. Download the planning sheet, fill in RO Numbers and packed quantities, then upload here.
            </div>
          )}
              ...
              <div className={TABLE_CLASSES.overflow}>
                <table className={TABLE_CLASSES.table}>
                  <thead className={TABLE_CLASSES.thead}>
                    <tr>
                      {['RO Number', 'Date', 'Location', 'Item ID', 'UPC/EAN', 'SKU Code', 'Item Name', 'MRP', 'SP', 'Requested Qty', 'Packed Qty', 'Status'].map(h => (
                  ...
                  <tbody className={TABLE_CLASSES.tbody}>
                    {filteredProc.map((row, idx) => (
                      <tr key={`${row.ro_number}-${row.sku_code}-${idx}`} className={TABLE_CLASSES.tr}>
                        <td className={TABLE_CLASSES.td}><span className='text-xs font-mono text-zinc-600 dark:text-zinc-400'>{row.ro_number}</span></td>
                        ...
                        <td className={TABLE_CLASSES.td}><span className='text-xs font-mono text-zinc-600 dark:text-zinc-400'>{row.upc_ean || '—'}</span></td>
                        ...
                        <td className={TABLE_CLASSES.td}>
                          <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${
                            row.status === 'Delivered' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                            row.status === 'Dispatch' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                            row.status === 'Packed' || row.status === 'packed' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' :
                            row.status === 'intransit' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                            row.status === 'processing' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' :
                            'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400'
                          }`}>
                            {row.status || '—'}
          ...
          <div className={TABLE_CLASSES.container}>
            <div className={TABLE_CLASSES.headerSection}>
              <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
                <div>
                  <p className='text-sm text-zinc-600 dark:text-zinc-400'>
                    Summary aggregated from the Processing tab by RO Number + Location. Edit <strong>Reason, Appointment dates, Dispatched date, Status</strong> inline.
                  </p>
                </div>
                <button
                  onClick={loadSummary}
              ...
              <div className='mt-4'>
                <SearchBar value={searchSum} onChange={setSearchSum} placeholder='Search by RO Number or location…' />
              </div>
              {summary.length > 0 && (
              ...
              <div className={TABLE_CLASSES.overflow}>
                <table className={TABLE_CLASSES.table}>
                  <thead className={TABLE_CLASSES.thead}>
                    <tr>
                      {[
                        'RO Number', 'PO Date', 'Location',
                        'Requested Qty', 'Accepted / Dispatched Qty',
                  ...
                  <tbody className={TABLE_CLASSES.tbody}>
                    {filteredSum.map(row => (
                      <SummaryEditRow
                        key={`${row.ro_number}|||${row.location}`}
                        row={row}
